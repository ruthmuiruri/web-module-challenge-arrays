...
function removeFlavorByName (arr_name, flavor){
  let abc=arr_name.indexOf(flavor);
  arr_name.splice (abc,1);
  return arr_name;
...
function filterByWord(arr_name, flavor){
  let arr_two = [];
  for (let i = 0; i < arr_name.length; i++) {
    if (arr_name [i].includes(flavor)) {
      arr_two.push(arr_name[i]);
